...
uniform sampler2D osc0_mod; // unused
...
uniform float width;
uniform float height; // unused

uniform float pm_weight; // unuused
...
void main() {
    ...
    vec4 mod_osc = texture2D(tex0, uv + vec2(.0025, .0025));

    float om_bright = .299 * mod_osc.r + .587 * mod_osc.g + .114 * mod_osc.b;

    vec4 cam1_color = texture2D(cam1, fract(uv + vec2(.005, .005)));
    /*
    vec2 axis=rotate(uv, osc1_angle);
    ...
    */
    ...
    float osc1_spiral_new = osc1_spiral * distance(uv, vec2(.5, .5));

    float osc1_m_spiral_new = osc1_m_spiral * distance(uv, vec2(.5, .5));

    vec2 axis = osc1_spiral_new + rotate(uv, osc1_angle + osc1_rotate + osc1_modulation_rotate * oscillate(osc1_m_cam1_pm * cam1_color.g + osc1_m_rate +
        +osc1_m_spiral_new + (rotate(uv, osc1_m_angle + osc1_m_rotate + osc1_m_modulation_rotate * oscillate(osc1_rate + rotate(uv, osc1_angle).y * (osc1_frequency), osc1_shape, osc1_width)).y + .01) * (osc1_m_frequency + .1 * osc1_m_pm * oscillate(osc1_rate + osc1_spiral_new + rotate(uv, osc1_angle + osc1_rotate).y * (osc1_frequency), osc1_shape, osc1_width)), osc1_m_shape, osc1_m_width));

    // +rotate(uv, osc1_m_angle+osc1_m_spiral*distance(uv, vec(.5,.5))+osc1_m_rotate+osc1_m_modulation_rotate*oscillate(osc1_rate+rotate(uv, osc1_angle).x*(osc1_frequency),osc1_shape,0)).x*(osc1_m_frequency),osc1_m_shape,0));

    m_axis = osc1_m_spiral_new + rotate(uv, osc1_m_angle + osc1_m_rotate + osc1_m_modulation_rotate * oscillate(osc1_rate + osc1_spiral_new + rotate(uv, osc1_angle + osc1_rotate).y * (osc1_frequency), osc1_shape, osc1_width));//+osc1_m_modulation_rotate*oscillate(osc1_rate+rotate(uv, +osc1_spiral*distance(uv, vec2(.5,.5))+osc1_rotate).x*(osc1_frequency),osc1_shape,0));

    float osc1_m_arg = osc1_m_rate + (m_axis.y + .01) * (osc1_m_frequency + .1 * osc1_m_pm * oscillate(osc1_rate + osc1_spiral_new + rotate(uv, osc1_angle + osc1_rotate).y * (osc1_frequency), osc1_shape, osc1_width));
